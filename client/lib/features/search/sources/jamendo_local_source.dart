  ...
  Future<List<SearchResult>> search(
    ...
    try {
      ...
      for (final effectiveQuery in buildMediaSearchQueries(query)) {
        for (final useNameSearch in const [true, false]) {
          final uri = Uri.https('api.jamendo.com', '/v3.0/tracks', {
            ...
            'audioformat': 'mp31',
            'include': 'musicinfo',
            if (useNameSearch) 'namesearch': effectiveQuery else 'search': effectiveQuery,
          });
          ...
          final results = items
              ...
              .map((item) {
                ...
                final downloadUrl = '${item['audiodownload'] ?? ''}'.trim();
                ...
                final tags = '${item['musicinfo']?['tags'] ?? ''}'.trim();
                ...
                return SearchResult(
                  ...
                  playUrl: audioUrl.isNotEmpty
                      ? audioUrl
                      : (downloadUrl.isNotEmpty ? downloadUrl : shareUrl),
                  ...
                  canonicalUrl: shareUrl.isNotEmpty ? shareUrl : audioUrl,
                  ...
                  supportsDownload: downloadUrl.isNotEmpty,
                );
              })
